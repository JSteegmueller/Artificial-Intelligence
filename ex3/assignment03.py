...
class DefineCity:
    def __init__(self, location, name, neighbours=[], dist_neighbours=[], parent=None, f=10000, sum_g=0):
        self.neighbours: List[DefineCity] = neighbours
        self.location = location
        self.dist_neighbours = dist_neighbours
        self.parent = parent
        self.name = name
        self.f = f
        self.sum_g: int = sum_g
    ...
    def h(self, city2):
        return math.sqrt(pow(self.location[0] - city2.location[0], 2) + pow(self.location[1] - city2.location[1], 2))
...
def find_path(start_name, goal_name, search_mode):
    ...
    start_city: DefineCity = cities[start_name]
    goal_city: DefineCity = cities[goal_name]
    ...
    frontiers: List[DefineCity] = [current_city]
    explored: List[DefineCity] = []
    # TODO: Expand node, update queues and reach goal city in this section
    while True:
        if len(frontiers) == 0:
            raise Exception("Error")
        frontiers = sorted(frontiers, key=lambda city: city.f)
        current_city = frontiers.pop(0)
        ...
        for neighbour in current_city.neighbours:
            ...
            neighbour.f = int(neighbour.sum_g + neighbour.h(goal_city) if search_mode == 'a*' else neighbour.sum_g)

            same_explored = None
            for idx, item in enumerate(explored):
                if item.name == neighbour.name:
                    same_explored = idx
                    break
            if same_explored:
                continue
            ...
            for idx, item in enumerate(frontiers):
                if item.name == neighbour.name:
                    same_frontier = idx
                    break
            if same_frontier and frontiers[same_frontier].sum_g >= neighbour.sum_g:
                frontiers[same_frontier].sum_g = neighbour.sum_g
                frontiers[same_frontier].f = neighbour.f
                frontiers[same_frontier].parent = copy.copy(current_city)
                continue

            neighbour.parent = copy.copy(current_city)
            frontiers.append(copy.copy(neighbour))
    # TODO: return path from start to goal:
    path = []
    reverse = explored.pop()
    while reverse:
        path.append(reverse.name)
